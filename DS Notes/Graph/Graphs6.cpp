...
min heap - dist,node
and we will have dist[]={initial infinity then update}
...
Using set -
set stores unique element and in ascending order
and when you get a better option of node and dist then you can erase previous one and 
replace it but in pq you can't do that you will have to traverse for it
...
/*
Shortest distance in a binary maze - Dijkstra using queue
Here we can even use queue instead of pq because here distance will be one unit
and there is no need to store them in the order of the distance , so simply use queue
...
as soon as you get dist for destination in queue stop then because coming future distance will be always
more then current one
...
return min effort of the path - effort is the max absoulute different between two consecutive element
